...
void DecisionTree(node *root,vector<string> y_label_class,double thresh_hold,vector<vector<string>> feature_class)
{
    ...
    if(used_dim.size()==(root->splited_data[0].size()-1))
    {
        ...
        {
            node *child_node= new node(splited_data[feature_num],0,feature_class[root->selected_dimention][feature_num],FindLatgestYlabel(splited_data[feature_num],y_label_class)); // generate new child_node
...
int main()
{
    ...
    string train_path="/home/fengchao/MachineLearning-CPP/my-code/statistics-learning-method-for-cpp/iris-data/watermelon.csv";
   ...
   // double ent1=ConditionalEntropy(my_data[1],y_label_class,1);
    double gain=GainInfo(my_data[1],y_label_class,1);
    vector<vector<string>> data = DataDiscreteByBiPartrition(my_data[1],y_label_class,1);
    //vector<vector<string>> data = SortByDim(my_data[1],1);// this is used for testing 
